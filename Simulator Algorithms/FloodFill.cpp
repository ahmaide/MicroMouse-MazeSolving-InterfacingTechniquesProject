...
map<pair<int, int>, char> previous;
...
void updateposition2(char currentMove);
...
void goBack(vector<vector<Cell>> &map);
...
void goBack(vector<vector<Cell>> &map){
    ...
    API::turnRight();
    updateposition('r');
    while(shortestPath.find({x,y}) != shortestPath.end()){
...
void updateposition(char currentMove)
{
    ...
    case 'N':
        if (currentMove == 'l')
            ...
            if((leastDistance.find({x,y+1}) == leastDistance.end()) ||
            leastDistance[{x,y+1}]> leastDistance[{x,y}] + 1){
                leastDistance[{x, y+1}] = leastDistance[{x, y}] + 1;
                previous[{x, y+1}] = 'S';
            }
            ++y;
    ...
    case 'E':
        if (currentMove == 'l')
            ...
            if((leastDistance.find({x+1,y}) == leastDistance.end()) ||
               leastDistance[{x+1,y}]> leastDistance[{x,y}] + 1){
                leastDistance[{x+1, y}] = leastDistance[{x, y}] + 1;
                previous[{x+1, y}] = 'W';
            }
            ++x;
...
void initializeMaze(vector<vector<Cell>> &map)
{
    ...
    for (int i = 0; i < mazeSize; i++)
    {
        for (int j = 0; j < mazeSize; j++)
        {
            ...
            API::setText(i, j, to_string(floodFillCost));
    ...
    while (!floodStack.empty())
    {
        ...
        if (cell.getHasBeenExplored())
        {
            ...
            if (map[cellX][cellY].getFloodFillCost() != 1 + minDistance)
            {
                ...
                API::setText(cellX, cellY, to_string(1 + minDistance));
        ...
        else
        {
            ...
            if (map[cellX][cellY].getFloodFillCost() != 1 + minD)
            {
                ...
                API::setText(cellX, cellY, to_string(1 + minD));
